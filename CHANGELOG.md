...
- `loco_rs::agui`: add durable, background-worker-driven runs. New `agui.execution` config (`inline` | `worker`): `worker` enqueues each run as a `agui::worker::RunAgentJob` on the background-worker queue so it survives a web-process restart and can be retried, while a web node still streams it through the DB-backed run hub. Defaults to `inline` (unchanged behavior). Requires `workers.mode: BackgroundQueue` and a DB-backed `agui.hub`; falls back to inline otherwise.
