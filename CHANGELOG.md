...
##  Unreleased
- Add a `/_metrics` monitoring endpoint (registered by default) that returns metrics in the Prometheus text exposition format. Loco emits a small set of always-available metrics and stays dependency-free: build/server info (`loco_build_info`, `loco_routes_total`, `loco_uptime_seconds`, `loco_start_time_seconds`) and Tokio runtime metrics from the stable `RuntimeMetrics` subset (`loco_runtime_workers`, `loco_runtime_alive_tasks`, `loco_runtime_global_queue_depth`) — no `--cfg tokio_unstable` required. Applications expose their own metrics (application, HTTP via `axum-prometheus`, or full `tokio-metrics`) by overriding the new `Hooks::metrics` hook (returns a Prometheus-formatted `String`, evaluated per scrape). Rendering lives in `loco_rs::metrics`.
