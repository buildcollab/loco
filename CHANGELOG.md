...
##  Unreleased
- Add a `/_metrics` monitoring endpoint (registered by default) that returns metrics in the Prometheus text exposition format. Loco emits a small set of always-available runtime metrics (`loco_build_info`, `loco_routes_total`, `loco_uptime_seconds`, `loco_start_time_seconds`) and stays dependency-free; applications expose their own metrics by overriding the new `Hooks::metrics` hook (returns a Prometheus-formatted `String`, evaluated per scrape). Rendering lives in `loco_rs::metrics`.
