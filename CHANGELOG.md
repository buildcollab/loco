...
##  Unreleased
- Add an opt-in `otel` feature that wires metrics through OpenTelemetry: an `SdkMeterProvider` backed by an `opentelemetry-prometheus` pull exporter, a `tracing-opentelemetry` `MetricsLayer` (so `tracing` metric events like `info!(monotonic_counter.foo = 1)` are recorded), and rendering of the Prometheus registry on the existing `/_metrics` endpoint. Includes an OpenTelemetry-instrumented HTTP middleware (`metrics::otel::HttpMetrics` + `track`) that records to native instruments. Off by default — the dependency-free `metrics::http` / `metrics::runtime` helpers remain for builds without the feature. Integration lives in `loco_rs::metrics::otel` (deps: `opentelemetry` 0.32, `opentelemetry_sdk` 0.32, `opentelemetry-prometheus` 0.32, `tracing-opentelemetry` 0.33, `prometheus` 0.14).
- Add a `/_metrics` monitoring endpoint (registered by default) that returns metrics in the Prometheus text exposition format. The core endpoint stays dependency-free and emits only always-available server metrics (`loco_build_info`, `loco_routes_total`, `loco_uptime_seconds`, `loco_start_time_seconds`). Applications expose their own metrics by overriding the new `Hooks::metrics` hook (returns a Prometheus-formatted `String`, evaluated per scrape). Rendering lives in `loco_rs::metrics`.
- Add two dependency-free metric helper libraries under `loco_rs::metrics` that plug into the `Hooks::metrics` hook — no extra crates or build flags: `metrics::http` (an Axum middleware collecting HTTP request counts, a latency histogram, and an in-flight gauge, labeled by method / matched route / status; install via `HttpMetrics::install` + `from_fn_with_state(.., http::track)`, render via `http::render`) and `metrics::runtime` (Tokio runtime metrics — `loco_runtime_workers`, `loco_runtime_alive_tasks`, `loco_runtime_global_queue_depth` — from the stable `RuntimeMetrics` subset). For richer metrics, render `axum-prometheus` / `tokio-metrics` output into the same hook (the latter's full set needs `RUSTFLAGS="--cfg tokio_unstable"`).
- Add a `/_server` monitoring endpoint (registered by default alongside `/_ping`, `/_health`, `/_readiness`) that returns a JSON manifest describing the running server: application name/version, environment, registered routes, and build-time metadata (`loco-rs` version, `rustc` version, build profile, target triple) captured by the build script. Applications can add custom fields by overriding the new `Hooks::server_info_extras` hook (returns a `serde_json::Value`, defaults to `null`). The manifest type lives in `loco_rs::server_info`.
- `loco_rs::agui`: fix `store::DbStore::load_history` replaying the current turn's in-progress assistant row. `run_turn` inserts the assistant message (status `"streaming"`) *before* loading history, so that still-empty placeholder was being fed back to the provider as a spurious empty assistant turn. History now skips in-progress `"streaming"` rows that carry no `parts` and no `content`, while keeping interrupt-finalized `"streaming"` rows (which hold the `tool_use` `parts` that `resume` replays).
- `loco_rs::agui`: add a `scope::contains(column, subset)` helper that builds a Postgres JSONB-containment (`@>`) condition, so a `ScopeResolver` can override the default exact-match `filter` to make richly-scoped conversations (e.g. `{organization_id, project_id}`) visible to coarser queries (an org-wide rail passing just `{organization_id}`) without hand-rolling raw SQL. `DbMemoryStore` visibility now uses the same containment on Postgres, so a memory stamped with a richer scope is retrievable from a coarser tenant context too (other backends keep exact equality). The generated agent migration also builds GIN indexes on the `conversations.scope` and `memories.scope` JSONB columns under Postgres so scoped reads stay index-backed.
- `loco_rs::agui`: move the agent runtime's DB-backed plumbing out of the generator and into the framework. The `ConversationStore` (`store::DbStore`), the multi-node run hub (`hub::DbRunHub` + `run_hub`), the config-driven provider / system-prompt factories (`service`), and the HTTP router (`controller::routes`) are now library code over framework-owned entities (`agui::entities`), behind the `with-db` feature. `cargo loco generate agent <name>` now scaffolds only the migration, the per-agent modules (prompt / tools / hooks), the registry, and a one-line controller mount — the previously generated `src/agents/runtime.rs` and `src/agents/store.rs` are gone.
- `loco_rs::agui`: add durable, background-worker-driven runs. New `agui.execution` config (`inline` | `worker`): `worker` enqueues each run as a `agui::worker::RunAgentJob` on the background-worker queue so it survives a web-process restart and can be retried, while a web node still streams it through the DB-backed run hub. Defaults to `inline` (unchanged behavior). Requires `workers.mode: BackgroundQueue` and a DB-backed `agui.hub`; falls back to inline otherwise. Docs at `docs/processing/agents.md`.
- `loco_rs::agui`: add `start_run` for headless agent runs — kick off an agent from a task, the scheduler, or another worker (e.g. generate a report asynchronously) with no HTTP request or client attached. It opens a conversation, seeds the prompt, and dispatches inline or onto the durable worker queue per `agui.execution`, returning the conversation/run ids. The trigger message is now seeded by the shared executor (`worker::dispatch_run` / `execute`), so the HTTP `POST /run` path and `start_run` behave identically.
- Add `loco_rs::cable` — realtime pub/sub for WebSocket / SSE channels (Loco's equivalent of Solid Cable). New `Cable` handle on `AppContext`, `Channel` trait + `ChannelRegistry`, Axum WS / SSE bridges (`cable::transport`), `Hooks::register_channels` (default no-op), and a `cargo loco generate channel <name>` template. Five backends behind feature flags (all in `default` except InMem which is always-on): `cable_pg`, `cable_sqlt`, `cable_redis`, `cable_pgmq`, plus the always-on InMem `tokio::sync::broadcast` provider. New `cable: Option<CableConfig>` section on `Config`. Docs at `docs/processing/cable.md`.
