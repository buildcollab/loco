...
impl MigrationTrait for Migration {
    async fn up(&self, m: &SchemaManager) -> Result<(), DbErr> {
        ...
        // Postgres: GIN-index the JSONB `scope` columns. Tenant-scoped reads
        // filter conversations (and memories) by `scope` — an exact match by
        // default, or `scope @> {..}` containment (see `loco_rs::agui::scope`).
        // Both are index-backed by a GIN index; without it every scoped read is
        // a sequential scan. Other backends have no JSONB index to build here.
        if matches!(
            m.get_database_backend(),
            sea_orm::DatabaseBackend::Postgres
        ) {
            for (table, col) in [("conversations", "scope"), ("memories", "scope")] {
                m.get_connection()
